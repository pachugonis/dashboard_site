...
К цели можно приложить изображение — оно появится на её карточке. Обрезка в
квадрат делается в браузере, в базу попадает готовая картинка до 512 КБ, так
что резервная копия базы забирает её вместе с историей.
