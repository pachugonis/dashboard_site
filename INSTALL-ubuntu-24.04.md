...
| Цель жёлтая, «с оговоркой» | сервис на связи, но отдаёт не то, чего ждёт цель: страница анти-DDoS, сломанный бэкенд, не тот путь. Причина написана прямо в карточке; лечится ожидаемым кодом в настройках цели либо режимом `tcp` |
| Цель зелёная, под ней «Редирект 302 на …» | так и задумано: капча встречает редиректом, и сервис за ней работает. Учтите, что `expect_text` по такой цели уже не проверяется — содержимое за редиректом монитор не видит |
