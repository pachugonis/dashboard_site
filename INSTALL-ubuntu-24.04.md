...
> **Не создавайте каталог `authorized_clients`, если ключи вам не нужны.**
> Само его существование включает клиентскую авторизацию, а пустой список
> разрешённых клиентов закрывает сервис для всех, включая вас. Симптом —
> адрес не открывается вообще, при этом Tor работает и дашборд отвечает на
> `127.0.0.1:8088`. Лечится так:
>
> ```bash
> sudo rmdir /var/lib/tor/onionwatch/authorized_clients
> sudo systemctl restart tor@default
> ```
...
Все три шага нужно доделать до конца: каталог `authorized_clients` без
`.auth`-файла внутри закрывает сервис для всех.
...
**5. Убедитесь, что список клиентов не пуст** — это самая частая ошибка:
...
Должен быть хотя бы один `.auth`-файл с непустой строкой
`descriptor:x25519:<52 символа base32 в верхнем регистре>`. Пустой каталог
означает «авторизация включена, разрешённых клиентов нет» — сервис не откроется
ни у кого. Если ключи в итоге не нужны, каталог надо удалить целиком
(`sudo rmdir …/authorized_clients`), а не оставлять пустым.
...
| Onion-адрес не открывается, при этом tor работает и `curl 127.0.0.1:8088` даёт `200` | скорее всего пустой каталог `authorized_clients`: он включает клиентскую авторизацию с пустым списком клиентов. Не нужны ключи — `sudo rmdir /var/lib/tor/onionwatch/authorized_clients && sudo systemctl restart tor@default` |
...
- [ ] Если клиентская авторизация не нужна — каталога `authorized_clients` нет
      (пустой каталог закрывает onion-сервис для всех)
