...
`admin.html`, `news.html`, `stats.html`, `config.example.json`, `onionwatch.service`,
`install.sh`.
...
| Путь | Что там |
|---|---|
| `/opt/onionwatch/` | код: `onionwatch.py`, `dashboard.html`, `admin.html`, `news.html`, `stats.html` (только чтение) |
...
```bash
scp onionwatch.py dashboard.html admin.html news.html stats.html config.example.json onionwatch.service \
...
sudo install -o root -g root -m 0644 stats.html     /opt/onionwatch/
...
У одной цели может быть несколько адресов: один обычный (clear) и до десяти
onion. Каждый проверяется отдельно, по своей цепочке Tor, и виден на карточке
отдельной строкой-ссылкой. Цель считается доступной, пока отвечает хоть один
из них, — так падение зеркала не выглядит падением сервиса.

К цели можно приложить изображение — оно появится на её карточке. Форматов три:
PNG и JPEG обрезаются в квадрат прямо в браузере (сторона до 256 пикселей, что
мельче — не растягивается), SVG сохраняется как есть и обрезается уже при показе.
В Tor Browser как есть уходит и растр: он запрещает читать canvas, и обрезать
картинку браузеру нечем — в квадрат её впишет вёрстка карточки. Предел — 5 МБ на
файл. Всё лежит в базе, поэтому резервная копия забирает картинки вместе
с историей.
...
когда цель упала и когда вернулась в норму. Отдельно он сообщает про упавшие
зеркала: цель с живым запасным адресом остаётся доступной, но знать об этом
стоит (см. раздел «Два состояния» в `README.md`).
...
def alive(last):
    """Состояний два: до сервиса достучались или нет."""
    if not last:
        return None                    # ещё ни разу не проверялся
    return (last.get("state") or ("up" if last.get("ok") else "down")) != "down"
...
for t in state["targets"]:
    ...
    if up is None:
        continue
    dead = [a["url"] for a in t.get("addresses", []) if alive(a["last"]) is False]
    if not up:
        bad[t["name"]] = "не отвечает"
    elif dead:
        # Цель жива за счёт запасного адреса — это не падение, но повод знать.
        bad[t["name"]] = "упало зеркало: " + ", ".join(dead)
...
# Пишем и когда цель стала нештатной, и когда сменила одну беду на другую:
# переход «упало зеркало» → «не отвечает» важнее самого первого сообщения.
if new := [n for n, why in bad.items() if prev.get(n) != why]:
    notify(f"[{host}] " + "; ".join(f"{n} — {bad[n]}" for n in sorted(new)))
...
Если оповещения об упавших зеркалах не нужны, уберите ветку `elif dead`.
После обновления первый запуск пришлёт по одному сообщению на каждую нештатную
цель: формат файла состояния изменился, и старый скрипт его не поймёт — это
разовое.
...
```bash
scp onionwatch.py dashboard.html admin.html news.html stats.html admin@<IP>:~/onionwatch/
...
sudo install -o root -g root -m 0644 ~/onionwatch/stats.html     /opt/onionwatch/
...
| Цель красная, а в Tor Browser открывается | красное значит «не достучались»: смотрите `error_slug` в логе. Код ответа цель не роняет — 503 и капча считаются доступностью. Разбор — в разделе «Ложная недоступность» в `README.md` |
| Цель красная, хотя сервис отвечает | скорее всего задана искомая строка, а в ответе её нет: `error_slug` будет `text_missing`. Уберите строку в настройках цели либо переведите её в режим `tcp` |
| Цель зелёная, но одно зеркало красное | так и задумано: цель доступна, пока отвечает хоть один адрес. Состояние каждого адреса видно в его строке на карточке, а фильтр «только проблемные» такие цели показывает |
